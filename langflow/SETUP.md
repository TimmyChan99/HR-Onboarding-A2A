...
Webhook or API Input
  ...
  -> Agent
       tool: A2A Dispatcher API Request
       tool: Backend Callback API Request
...
```text
Method: POST
URL: https://YOUR-A2A-HOST/orchestrator/dispatch
Content-Type: application/json
X-A2A-API-Key: secret configured outside the prompt
```

The tool body is generated by the Orchestrator and must match `DispatchRequest`:
...
Do not put the service host, API key, Langflow flow IDs, or Langflow API key in the LLM prompt.
